...
class _InterviewRecordingPageState extends State<InterviewRecordingPage>
  ...
  Future<void> _goResult() async {
    ...
    // ===== 임시 더미 결과 생성 (총점 40 기준) =====
    final now = DateTime.now();
    final rnd = math.Random();

    // 1) 인지 지표(리스크바 UI에서 쓰는 6개 키) - 각 지표 최소 1점 배정 + 총합 40
    const cogKeys = <String>[
      '반응 시간',
      '반복어 비율',
      '평균 문장 길이',
      '화행 적절성',
    ...
    // 40점을 6개로 랜덤 분할 (각 최소 1점 보장)
    List<int> _splitTotal(int sum, int parts) {
      // 1..(sum-1) 중 중복 없이 컷 5개 뽑아서 구간 분할
      final cutSet = <int>{};
      while (cutSet.length < parts - 1) {
        cutSet.add(rnd.nextInt(sum - 1) + 1); // 1..sum-1
      }
      final cuts = cutSet.toList()..sort();
      final out = <int>[];
      int prev = 0;
      for (final c in cuts) {
        out.add(c - prev);
      ...
      out.add(sum - prev);
      // 혹시라도 0이 생기면(이론상 안 생기지만 방어) 1로 보정
      for (int i = 0; i < out.length; i++) {
        if (out[i] <= 0) out[i] = 1;
      }
      // 합 보정
      final diff = sum - out.fold<int>(0, (s, e) => s + e);
      if (diff != 0) {
        final idx = rnd.nextInt(out.length);
        out[idx] = (out[idx] + diff).clamp(1, sum - (parts - 1));
      }
      return out;
    }

    final totals = _splitTotal(40, cogKeys.length);

    // 각 지표별 정답률을 0.40~0.90 사이로 랜덤 설정 → correct 계산
    final byCategory = <String, CategoryStat>{};
    int scoreSum = 0;
    for (int i = 0; i < cogKeys.length; i++) {
      final t = totals[i];
      final acc = 0.40 + rnd.nextDouble() * 0.50; // 40% ~ 90%
      int correct = (t * acc).round().clamp(0, t);
      byCategory[cogKeys[i]] = CategoryStat(correct: correct, total: t);
      scoreSum += correct;
    }

    // 2) 동화 타입(서버 페이로드/그래프용) 4개도 임의 값 구성 (총합은 임의 16점)
    const storyKeys = <String>['요구', '질문', '단언', '의례화'];
    final storyTotals = _splitTotal(16, storyKeys.length);
    final byType = <String, CategoryStat>{};
    for (int i = 0; i < storyKeys.length; i++) {
      final t = storyTotals[i];
      final acc = 0.50 + rnd.nextDouble() * 0.40; // 50% ~ 90%
      int correct = (t * acc).round().clamp(0, t);
      byType[storyKeys[i]] = CategoryStat(correct: correct, total: t);
    }

    // ===== 결과 페이지로 이동 (총점 40, 점수=각 지표 correct 합계) =====
    await Navigator.of(context).pushReplacement(
      PageRouteBuilder(
        pageBuilder:
            (_, __, ___) => InterviewResultPage(
              score: scoreSum, // 무작위로 계산된 정답 합계
              total: 40, // 총점 40 고정
              byCategory: byCategory, // 6개 인지 지표 (리스크바에 직접 반영됨)
              byType: byType, // 동화 타입 4개 (서버 페이로드용)
              testedAt: now,
              interviewTitle: '인지 능력 검사(임시)', // 표시용
            ),
        transitionDuration: Duration.zero,
        reverseTransitionDuration: Duration.zero,
        transitionsBuilder:
