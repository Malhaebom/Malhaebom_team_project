...
function rowToCardData(row) {
  ...
  const displayTime = client_kst?.trim() ? client_kst : toKstString(row?.client_utc || "");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 쿠키 인증이 있다 해도 params/header로 user_key명시
