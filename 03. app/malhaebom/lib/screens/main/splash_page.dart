...
class _SplashPageState extends State<SplashPage>
  ...
  static const bool logoFromNative = false;
  ...
  Widget build(BuildContext context) {
    ...
    final fixedMedia = MediaQuery.of(context).copyWith(
      textScaler: const TextScaler.linear(1.0), // 최신 플러터
    );
...
class _AngleBoldText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final m in reg.allMatches(text)) {
      if (m.start > i)
        spans.add(TextSpan(text: text.substring(i, m.start), style: style));
