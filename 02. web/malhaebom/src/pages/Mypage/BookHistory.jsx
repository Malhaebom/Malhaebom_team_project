...
function normalizeScores({ by_category, by_type, risk_bars, risk_bars_by_type }) {
  ...
  const fromRatio = (ratio) => {
    // ratio: 0~1 (위험도) → correct ≈ (1 - ratio) * 4
  ...
  // A/AI는 by_type(‘직접화행/간접화행’) 우선
  const A_fromType  = getCorrect(by_type, "직접화행");
  const AI_fromType = getCorrect(by_type, "간접화행");

  // B/C/D: by_category에서 영문 또는 국문 키
  const B_fromCat = getCorrect(by_category, "B") ?? getCorrect(by_category, "질문");
  const C_fromCat = getCorrect(by_category, "C") ?? getCorrect(by_category, "단언");
  const D_fromCat = getCorrect(by_category, "D") ?? getCorrect(by_category, "의례화");

  // 부족하면 risk_bars_by_type / risk_bars에서 보강
  const A =
    A_fromType ??
    fromRatio(risk_bars_by_type?.["직접화행"]) ??
    fromPoints(risk_bars?.A);

  const AI =
    AI_fromType ??
    fromRatio(risk_bars_by_type?.["간접화행"]) ??
    fromPoints(risk_bars?.AI);

  const B =
    B_fromCat ??
    fromRatio(risk_bars?.["질문"]) ??
    fromPoints(risk_bars?.B) ?? 0;

  const C =
    C_fromCat ??
    fromRatio(risk_bars?.["단언"]) ??
    fromPoints(risk_bars?.C) ?? 0;

  const D =
    D_fromCat ??
    fromRatio(risk_bars?.["의례화"]) ??
    fromPoints(risk_bars?.D) ?? 0;

  return {
    scoreAD: Number.isFinite(A) ? A : 0,
    scoreAI: Number.isFinite(AI) ? AI : 0,
    scoreB:  Number.isFinite(B) ? B : 0,
    scoreC:  Number.isFinite(C) ? C : 0,
    scoreD:  Number.isFinite(D) ? D : 0,
  };
}
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 항상 user_key를 명시 전달(쿠키 의존 최소화), guest는 붙이지 않음
        const cfg = {};
        if (userKey && userKey !== "guest") {
          cfg.params = { user_key: userKey };
          cfg.headers = { "x-user-key": userKey };
        }
